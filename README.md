...
表現方針は 16ビット世代ゲーム機（スーパーファミコン相当）を基準にしています。
...
全機能解放モード（バランス/段階解禁を無視）:

```bash
python block_breaker.py --all-features
```
...
- 1面はチュートリアル面で、敵は出ません（2面以降で敵ON時に出現）
- `TAB`: Protocol切替（開始前）
  - `Fusion` / `Reflex`
...
`--all-features` モード時:
- 1面から敵・可動ブロック・硬いブロック・全アイテム（W/S/M/F）を有効化
- タイトル開始時にチュートリアルをスキップして即プレイ開始
...
- ライフが減った時は理由モーダルを表示し、`SPACE/ENTER` で再開します。
